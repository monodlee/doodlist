	...
	if (ctx.measureText(text).width < maxWidth) {
		...
		while (words.length > 0) {
			if (ctx.measureText(line + words[0]).width < maxWidth) {
				line += words.shift() + " ";
			} else {//if line is too long
				// If the line is empty and the word is too long to fit on line, add to line
				if (ctx.measureText(words[0]).width >= maxWidth && line.length === 0){
					line = words.shift();
				}
				// break line
				lines.push(line);
